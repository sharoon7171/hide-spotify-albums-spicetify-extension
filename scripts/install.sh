...
EXT_NAME="spotify-customization.js"
BOOTSTRAP_NAME="hidden-albums-bootstrap.js"
POST_SNAPSHOT_NAME="hidden-albums-post-snapshot.js"
...
BOOTSTRAP_TAG="extensions/hidden-albums-bootstrap.js"
POST_SNAPSHOT_TAG="extensions/hidden-albums-post-snapshot.js"
...
if command -v spicetify >/dev/null 2>&1; then
  spicetify config extensions "$EXT_NAME" 2>/dev/null || true
  spicetify apply
else
...
if [[ -f "$XPUI_INDEX" ]]; then
  ...
  perl -i -0pe "s|<script <script defer src='extensions/hidden-albums-bootstrap.js'></script>||g" "$XPUI_INDEX"
  if ! grep -q "hidden-albums-bootstrap.js'></script>${MODULES_TAG}" "$XPUI_INDEX"; then
